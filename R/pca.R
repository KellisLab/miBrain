...
#' @export
PlotPCA <- function(se, title=NULL, method="TMM", top=500, cpm.frac=0.25, cpm.cutoff=100, gene.selection="common", correct=FALSE, use_label=TRUE, force=0.2, text.size=4, max.overlaps=10) {
    require(ggplot2)
    title = ifelse(is.null(title), S4Vectors::metadata(title)$comparison, title)
    ...
    pf$x = pl$x
    pf$y = pl$y
    if (!("batch" %in% names(pf)) | (length(unique(pf$batch))==1)) {
